...
uses MacTypes,CFBase,CFArray,CGBase,CGColorSpace;
{$endc} {not MACOSALLINCLUDE}

{$ALIGN POWER}


// CGGradientRef defined in CGBase
...
function CGGradientGetTypeID: CFTypeID; external name '_CGGradientGetTypeID';
...
function CGGradientCreateWithColorComponents( space: CGColorSpaceRef; {const} components: {variable-size-array} CGFloatPtr; {const} locations: {variable-size-array} CGFloatPtr; count: size_t ): CGGradientRef; external name '_CGGradientCreateWithColorComponents';
(* CG_AVAILABLE_STARTING(__MAC_10_5, __IPHONE_2_0) *)
...
function CGGradientCreateWithColors( space: CGColorSpaceRef; colors: CFArrayRef; {const} locations: {variable-size-array} CGFloatPtr ): CGGradientRef; external name '_CGGradientCreateWithColors';
(* CG_AVAILABLE_STARTING(__MAC_10_5, __IPHONE_2_0) *)

{ Equivalent to `CFRetain' except that it doesn't crash (as `CFRetain'
   does) if `gradient' is NULL. }

function CGGradientRetain( gradient: CGGradientRef ): CGGradientRef; external name '_CGGradientRetain';
(* CG_AVAILABLE_STARTING(__MAC_10_5, __IPHONE_2_0) *)

{ Equivalent to `CFRelease' except that it doesn't crash (as `CFRelease'
   does) if `gradient' is NULL. }

procedure CGGradientRelease( gradient: CGGradientRef ); external name '_CGGradientRelease';
(* CG_AVAILABLE_STARTING(__MAC_10_5, __IPHONE_2_0) *)
